...
void OnPaint(HWND hWnd) {
    ...
    if (g_ctx.showFilePath) {
        ...
        } else if (g_ctx.currentImageIndex >= 0 && g_ctx.currentImageIndex < g_ctx.imageFiles.size()) {
        ...
        if (!pathToDisplay.empty()) {
            ...
            HFONT hOldPathFont = (HFONT)SelectObject(memDC, hPathFont);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam) {
    ...
    switch (message) {
        ...
        case WM_COPYDATA: {
            ...
            if (pcds->dwData == 1) {
                LoadImageFromFile((wchar_t*)pcds->lpData);
                GetImagesInDirectory((wchar_t*)pcds->lpData);
            }
            return TRUE;
        }
        case WM_SIZE:
